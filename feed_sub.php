...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["ajax"])) {
    ...
    if ($action === "loadPosts") {
        ...
        // НОВЫЙ SQL-ЗАПРОС
        // Выбираем посты, где автор - это сам пользователь ИЛИ автор есть в списке подписок пользователя.
        ...
        // НОВАЯ ПРИВЯЗКА ПАРАМЕТРОВ: ID текущего пользователя используется дважды
        $stmt->bind_param("iiii", $currentUserId, $currentUserId, $limit, $offset);
...
function loadPosts(reset = false){
    ...
    $.post('feed_sub.php', {ajax:1, action:'loadPosts', offset:offset}, function(posts){
...
function loadReplies(parentId){
    ...
    $.post('feed_sub.php', {ajax:1, action:'loadReplies', post_id: parentId}, function(res){
...
$(document).on('click', '.like-btn', function(){
    ...
    $.post('feed_sub.php', {ajax:1, action:'toggleLike', post_id:id}, function(res){
        res = JSON.parse(res);
        if(res.success){
...
$(document).on('click', '.dislike-btn', function(){
    ...
    $.post('feed_sub.php', {ajax:1, action:'toggleDislike', post_id:id}, function(res){
        res = JSON.parse(res);
        if(res.success){
...
$(document).on('click', '.delete-post-btn', function(e) {
    ...
    if (confirm("Вы уверены, что хотите удалить этот пост?")) {
        $.post('feed_sub.php', { ajax: 1, action: 'deletePost', post_id: postId }, function(res) {
            res = JSON.parse(res);
            if (res.success) {
...
$(document).on('click', '.report-post-btn', function(e) {
    e.preventDefault();
    const postId = $(this).data('id');
    $.post('feed_sub.php', { ajax: 1, action: 'reportPost', post_id: postId }, function(res) {
        res = JSON.parse(res);
        if (res.success) {
...
$(document).on('click', '.repost-btn', function(){
    ...
    $.post('feed_sub.php', {ajax:1, action:'addPost', original_post_id: originalId}, function(res){
        res = JSON.parse(res);
        if(res.success){
            // Передаем current_user_id в renderPost
            $('#posts-container').prepend(renderPost(res.post, false, false, current_user_id));
